 ...
 * Analyzes JavaScript/TypeScript code using AST to detect security risks
 */
...
import { Finding, RiskPattern } from './types';

// Risk patterns to detect in code
...
/**
 * Recursively find all JS/TS files in directory
 */
export function findSourceFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (/\.(js|ts|mjs|cjs)$/.test(entry.name) && !entry.name.endsWith('.d.ts')) {
        files.push(fullPath);
      }
    }
  }
...
function parseCode(source: string): acorn.Node | null {
  try {
    ...
    try {
      ...
      // For MVP, we'll skip files that can't be parsed
      return null;
    }
  }
}
...
export function analyzeFile(filePath: string): Finding[] {
  ...
  // This helps parse basic TS files without a full TS parser
 ...
 */
export function analyzeDirectory(targetDir: string): { findings: Finding[]; scannedFiles: number } {
